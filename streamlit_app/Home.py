...
modules = [
    ...
    ("10. SAP Interview Coach", "Available", "Practise SAP interview questions across nine tracks and six interview modes against a bundled bank of 104 fictional questions, each with an explicit rubric. Every score - overall, technical accuracy, completeness, clarity, business understanding and architecture where relevant - is calculated from that rubric by deterministic Python, with the matched keyword shown for every concept; AI only writes the coaching prose. Sessions, answers, times and feedback are stored, and a performance dashboard reports scores by topic, difficulty and track over time, with weak areas and a study plan derived from the answers themselves."),
]

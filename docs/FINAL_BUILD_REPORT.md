...
**188 Python modules in `app/`, 133 endpoints, 33 tables, 10 migrations, 1,627 tests.**
...
**133 endpoints in 11 tag groups.** Every one has a summary and a description;
...
| Supplier Risk Copilot | 13 | upload → calculate → suppliers → chat |
...
| SAP Interview Coach | 10 | start → answer → complete, questions, sessions, performance |
...
```text
1,627 passed in ~3 min
```

| Suite | Tests | Asks |
| --- | --- | --- |
| `tests/unit` | 800 | Does this rule, metric or model do the right arithmetic? |
| `tests/api` | 409 | Does this endpoint behave — status codes, validation, persistence? |
| `tests/integration` | 300 | Does the engine find everything the manifest says is there? Do the migrations, the deployment files and the docs still match? |
| `tests/e2e` | 118 | Does the *journey* work, and do the ten modules agree? |

Phase 5 added 178 tests, almost all of them cross-cutting: one workflow per
module, and five contract suites that read the generated OpenAPI document so a
new module is covered the moment it registers a route.
...
| 7 | All APIs are documented | **Yes** — 133 endpoints, every one with a summary and description; errors documented; collection generated |
...
| 10 | Reports download successfully | **Partly** — eight of ten modules export (XLSX/CSV/JSON, plus PDF, DOCX and Markdown where they fit). Modules 5 and 10 have none. See below. |
| 11 | Tests pass | **Yes** — 1,627 |
...
introduced, and building two new export pipelines is module work rather than
hardening — so it is recorded as a known limitation instead of being quietly
counted as a pass. Both modules' data is fully available over the API, so a
front end can render or export it today.
...
3. **Modules 5 and 10 have no export.** See criterion 10.
4. **File storage is local disk.** Fine on a VM with a volume, wrong on an
   ephemeral container. The presigned-URL shape is sketched in the integration
   doc.
5. **`approved_by` is free text.** Modules 8, 9 and 10 record who approved
   something as a string a caller supplies. That is a demo, not an audit trail;
   it becomes a foreign key when users exist.
6. **Keyword matching has a ceiling.** Module 10's rubric credits a concept when
...
7. **Forecast quality is bounded by the demo history.** 30 monthly periods is
...
8. **No OCR by default.** A scanned contract is reported as `needs_ocr` rather
   than analysed as an empty one. The three provider seams exist and are
   unconfigured.
9. **PostgreSQL is verified, not exercised.** The migrations render, the DDL is
...
10. **Single-currency assumptions in places.** Spend converts to a base currency
